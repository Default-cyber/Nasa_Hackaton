...
const App = () => {
  ...
  const fetchNASAPowerData = async () => {
    try {
      ...
      alert(translations[language].language === 'pt' ? 'Não foi possível carregar os dados da NASA. Usando perguntas padrão.' : 'Failed to load NASA data. Using default questions.');
      setQuestions(generateDynamicQuestions({ // Fallback data
        annualRainfall: 1200, monthlyRainfall: 100, avgTemp: 25, maxTemp: 32, ndvi: 0.65, soilMoisture: 50, rainfall5Years: [1150, 1300, 980, 1400, 1200], location: { lat: 0, lon: 0 }
      }));
      setGameState('quiz');
    }
  };
    ...
    const questionsTemplates = {
      pt: [
        ...
        },
        {
          ...
          correct: realData.soilMoisture < 40 && realData.maxTemp > 28 ? 1 : 2,
          explanation: {
            correct: `✅ Com umidade ${realData.soilMoisture}% e ${realData.maxTemp}°C, ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? 'irrigação leve em horários frescos é ideal' : 'pode aguardar'}.`,
            wrong: `❌ Umidade ${realData.soilMoisture}% com ${realData.maxTemp}°C: ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? 'Precisa irrigar!' : 'Está adequado!'} Irrigar no calor perde 40-60% por evaporação.`
          },
      ...
      en: [
        ...
        },
        {
          ...
          correct: realData.soilMoisture < 40 && realData.maxTemp > 28 ? 1 : 2,
          explanation: {
            correct: `✅ ${realData.soilMoisture}% moisture, ${realData.maxTemp}°C: ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? 'light irrigation ideal' : 'can wait'}.`,
            wrong: `❌ ${realData.soilMoisture}% with ${realData.maxTemp}°C: ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? 'Needs water!' : 'Adequate!'}`
          },
      ...
      es: [
        ...
        },
        {
          ...
          correct: realData.soilMoisture < 40 && realData.maxTemp > 28 ? 1 : 2,
          explanation: {
            correct: `✅ ${realData.soilMoisture}% humedad, ${realData.maxTemp}°C: ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? 'riego ligero ideal' : 'puede esperar'}.`,
            wrong: `❌ ${realData.soilMoisture}% con ${realData.maxTemp}°C: ${realData.soilMoisture < 40 && realData.maxTemp > 28 ? '¡Necesita agua!' : '¡Adecuado!'}`
          },
          dataSource: `NASA SMAP/POWER`
        },
        {
  ...
  if (gameState === 'language') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-700 to-blue-900 flex items-center justify-center p-4 font-sans">
  ...
  if (gameState === 'loading') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-700 to-blue-900 flex items-center justify-center font-sans">
  ...
  if (gameState === 'quiz' && questions.length > 0) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-green-700 to-blue-900 p-4 py-8 font-sans">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-2xl p-4 mb-6">
            ...
            <div className="relative rounded-xl overflow-hidden border-4 border-green-500 h-64 bg-gradient-to-br from-green-800 to-green-600">
              ...
              <div className="absolute top-3 left-3 bg-black bg-opacity-70 px-3 py-2 rounded-lg text-white text-xs">📍 {parseFloat(location.lat).toFixed(2)}, {parseFloat(location.lon).toFixed(2)}</div>
            </div>
          </div>
